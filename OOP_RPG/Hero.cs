...
namespace OOP_RPG
{
    public class Hero
    {
        ...
        public void LoadHero(string[] hero)
        {
            if (hero?[0] == string.Empty)
                return;
            this.Name = hero[0];
            this.CurrentHP = Convert.ToInt32(hero[1]);
            this.GoldCoin = Convert.ToInt32(hero[2]);
        }
        public string GetHero()
        {
            string heroData = string.Empty;
            heroData += string.Format($"{this.Name}\n{this.CurrentHP}\n{this.GoldCoin}\n");
            return heroData;
        }
        public string GetWeapon()
        {
            string weaponData = string.Empty;
            weaponData += string.Format($"{this.EquippedWeapon.Name}\n{this.EquippedWeapon.Strength}\n{this.EquippedWeapon.Price}\n");
            return weaponData;
        }
